...
class HumanPlayer:
    ...
    def decide_move(self, board, verbose=True, total_moves=0):
        """
...
class GreedyPlayer:
    ...
    def decide_move(self, board, verbose=False, training=False, total_moves=0):
        valid_moves = board.get_valid_moves(self.player_num)
        human_valid_moves = board_utils.convert_np_to_human_moves(valid_moves)
...
class AiPlayer:
    ...
    def decide_move(self, board, verbose=False, total_moves=0):
        """
        Given current board, return a move to play.
        :type board: Class Board
        :rtype A list of 2 tuples, specifying the move's FROM and TO.
        ...
        tree_tau = 0.01 if total_moves > TOTAL_MOVES_TILL_TAU0 else TREE_TAU
